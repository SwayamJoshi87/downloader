...
async function fetchPaste(baseUrl: string, pasteId: string): Promise<PasteResponse> {
  ...
  const json = (await res.json()) as PasteResponse;
...
export async function decryptPaste(pasteUrl: string): Promise<string> {
  ...
  const parsed = JSON.parse(text) as { paste: string };
  return parsed.paste;
}
