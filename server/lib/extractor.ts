...
export type ExtractorCallback = (event: ExtractorEvent) => void;
...
function isRarPart(fileName: string): boolean {
...
function getPartNumber(fileName: string): number | null {
  ...
  // Plain .rar
  if (lower.endsWith('.rar') && !/\.part\d+\.rar$/.test(lower)) return 1;

  return null;
}

function isFirstRarPart(fileName: string): boolean {
  return getPartNumber(fileName) === 1;
}

function getRarSetBaseName(fileName: string): string | null {
...
function groupRarSets(files: string[]): RarSet[] {
  const sets = new Map<string, RarSet>();

  for (const file of files) {
    if (!isRarPart(file)) continue;
...
async function findRarExtractor(): Promise<string | null> {
...
function runExtractor(exe: string, args: string[], cwd: string): Promise<void> {
...
export async function extractAndOrganize(
  downloadFolder: string,
  outputFolder: string,
  onEvent: ExtractorCallback
): Promise<void> {
  ...
  for (let i = 0; i < rarSets.length; i++) {
    ...
    try {
      ...
      const msg = `Failed to extract ${set.firstPart}: ${(err as Error).message}`;
  ...
  for (const file of otherFiles) {
    ...
    try {
      ...
      const msg = `Failed to move ${file}: ${(err as Error).message}`;
      console.error('[extractor]', msg);
      onEvent({ type: 'error', message: msg });
      errorMessages.push(msg);
    }
